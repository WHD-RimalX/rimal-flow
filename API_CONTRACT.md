...
> **تحديث**: تم استلام `INTEGRATION_STANDARDS.pdf` الرسمي. راجع **§10 مطابقة معايير التكامل** أسفل هذا
> المستند لجدول الفروقات الكاملة بين نموذج بياناتنا الحالي والشكل المطلوب رسمياً، وخطة الترحيل المقترحة.
> القرار الحالي (بالاتفاق مع صاحب المشروع): توثيق الفروق الآن دون تعديل المخطط الداخلي (Prisma schema)
> فوراً، لعدم تعطيل النشر والاختبار اليوم — الترحيل الفعلي مجدول كخطوة تالية منفصلة.
...
هذا القسم يقارن نموذج بياناتنا الفعلي (المبني حالياً ويعمل) بالشكل الرسمي المطلوب في وثيقة معايير
التكامل، ويوثّق كل نقطة تطابق وكل نقطة فرق كخطوة ترحيل (Migration) لاحقة صريحة. **لا شيء من هذا نُفِّذ
بعد** — هذا سجل قرار (ADR مصغّر) لتفادي فقدان السياق بين الآن ووقت الترحيل الفعلي.
...
### 10.2 الفروقات — تحتاج قراراً وترحيلاً لاحقاً ⚠️
...
| **شكل الحجز (تاريخ/وقت)** | `startDate/endDate` (تاريخ فقط) + `startTime/endTime` منفصلان كنص `"HH:MM"` | `startTime/endTime` كـ `DateTime` كامل واحد لكل طرف | نموذجنا أبسط لحساب المدة والتعارضات مباشرة عبر مقارنة `DateTime`؛ التحويل للشكل المطلوب هو دمج/تفكيك تاريخ+وقت عند نقطة التصدير فقط، لا يمس منطق العمل الداخلي. |
| **أسماء حقول تسعير الحجز** | `originalPrice / totalPrice` | `basePrice / finalPrice` | `discountAmount` متطابق فعلاً؛ البقية إعادة تسمية فقط (Alias) عند التصدير. |
